...
## Visibility-gate prototype

This branch adds an opt-in SQL visibility gate and a paired WebSocket benchmark
for Realtime on PostgreSQL and Multigres. See the
[prototype instructions](visibility/README.md) and
[benchmark and validation report](results/visibility-gate/report.md).
The gate is installed only when `BENCH_VISIBILITY_GATE=true`; normal Realtime
