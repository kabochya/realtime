  ...
  test "database to Realtime processing and delivery" do
    ...
    connections =
      for _ <- 1..concurrency do
        {:ok, conn} = Database.connect_db(%{settings | pool_size: 1}, parameters: [application_name: settings.application_name, synchronous_commit: "local"])
        %{rows: [["local"]]} = Postgrex.query!(conn, "SHOW synchronous_commit", [])
        conn
      end
